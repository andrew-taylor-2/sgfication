...
const Board = ({ sgfData }) => {
  useEffect(() => {
    ...
    const loadWGoScripts = async () => {
      ...
      const player = new WGo.BasicPlayer(boardElement, {
        ...
        enableEdit: true // Initialize with edit mode enabled
      });

      // Wait for the player to be fully initialized -- very important
      if (player && player.kifu) {
        const editable = new WGo.Player.Editable(player, player.board);
        editable.set(true); // Enable edit mode
      } else {
        console.error("Player initialization failed. SGF data might be invalid.");
      }
    };
